...
async def fetch_base_league_data():
    """
...
async def fetch_matchups_up_to_week(current_week: int):
    """
    Helper to concurrently fetch matchup data from week 1 to current_week.
    
    Returns: A list of matchup data for each week.
    """
    tasks = [client.get_matchups(wk) for wk in range(1, current_week + 1)]
    matchups = await asyncio.gather(*tasks)
    return matchups
        
        
async def fetch_projections_up_to_week(current_week: int):
    """
    ...
    """
    tasks = [client.get_weekly_projections(wk) for wk in range(1, current_week + 1)]
...
async def fetch_rankings(week: int):
    league_data = await fetch_base_league_data()
    rosters_data, projections_to_week, matchups_up_to_week = await asyncio.gather(
        client.get_league_rosters(),
        fetch_projections_up_to_week(week),
        fetch_matchups_up_to_week(week),
    )
...
@app.get("/trends/{target_owner_name}/{week}")
async def fetch_team_trends(target_owner_name: str, week: int):
    league_data = await fetch_base_league_data()
    rosters_data, projections_to_week, matchups_up_to_week = await asyncio.gather(
        client.get_league_rosters(),
        fetch_projections_up_to_week(week),
